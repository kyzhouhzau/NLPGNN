...
# R8,R52
data = Sminarog(data="R8", data_dir="data", embedding="glove{}".format(glove_dim))
nodes, adjs, edge_attrs, labels, batchs, node2index = data.load_graph(mode="train")
nodes1, adjs1, edge_attrs1, labels1, batchs1,  _ = data.load_graph(mode="test")
# print(node2index["champion"])
# print(nodes[:2])
# print(adjs[:2])
# print(edge_attrs[:2])
# print(labels[:2])
...
class TextGCNDynamicWeight(tf.keras.layers.Layer):
    ...
    def call(self, node, adj, edge_attr, batch, training=True):
        ...
        predict = self.model(feature, etans, adj, batch, edge_attr, training=training)
        return predict
...
for i in range(10):
    ...
    for i in range(epoch):
        ...
        acc_train = []
        t = time.time()
        for node, label, adj, edge_attr, batch in load(nodes[:-500],
                                                       adjs[:-500], labels[:-500],
        ...
        t_loss = t_acc = 0
        for node, label, adj, edge_attr, batch in load(nodes[-500:], adjs[-500:],
    ...
    pres = []
    labs = []
    for node, label, adj, edge_attr, batch in load(nodes1, adjs1, labels1, edge_attrs1,
